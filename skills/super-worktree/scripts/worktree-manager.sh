...
cmd_create() {
  _require_in_repo
  local branch="" from_branch="" custom_config="" cli_tool="" ticket="" slug="" from_pr="" from_mr=""
  ...
  if [[ -n "$from_mr" ]]; then
    require_cmd glab "Install GitLab CLI (https://gitlab.com/gitlab-org/cli) to use --from-mr"
    [[ -z "$branch" ]] && branch="mr-$from_mr"
    log "Fetching MR #$from_mr ref via glab..."
    glab mr checkout "$from_mr" >/dev/null 2>&1 || die "glab mr checkout failed for #$from_mr"
